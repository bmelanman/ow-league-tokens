...
def shut_down_pc():
    ...
    for i in reversed(range(1, 6)):
        log_info(
            src, f'&rThis PC will shut down in &y{i} &rsecond{"s" if i > 1 else ""}!'
        )
        sleep(1)
...
def check_logged_in(driver: uc.Chrome | None = None) -> bool:
    """Verifies if a user is logged in to their YouTube.

    Args:
        driver (uc.Chrome | None, optional): Chromium instance, defaults to None.
...
def get_seconds_till_next_match() -> float | None:
    ...
    try:
        ...
        next_data = (
            schedule_html.split('<script id="__NEXT_DATA__" type="application/json">')[
                1
            ]
            .split("</script>")[0]
            .strip()
        ...
        pending_matches = list(
            filter(lambda match: match.get("status") == "PENDING", matches)
        )
        ...
        log_error(
            src, f"&rSchedule check failed: {str(e)}. Falling back to regular checks."
        )
...
def check_for_new_version():
    ...
    if response.status_code == 200 and get_version(latest_version) > get_version(
        CURRENT_VERSION
    ):
        log_info(
            log_src,
            f"&gNew version available! You are on version &r{CURRENT_VERSION}&g, "
...
log_error = lambda src, msg: logging.error(
    get_console_message(f"&c({src})&r " + msg + "&!r")
)
log_info = lambda src, msg: logging.info(
    get_console_message(f"&c({src})&y " + msg + "&!r")
)
log_debug = lambda src, msg: logging.debug(
    get_console_message(f"&c({src})&y " + msg + "&!r")
)
