...
def start_chrome(config: dict):
    ...
    if config["debug"]:
        live_src = TEST_CHANNEL_ID
    else:
        live_src = OWL_CHANNEL_ID
    ...
    while True:
        ...
        if config["enable_owl"]:
            url = get_active_stream(live_src, live_check_driver)
        ...
        if current_url != live_url:
            ...
            if current_url:
                ...
                for index, driver in enumerate(drivers):
                    ...
                    # Wait for stream to load before running traffictoll
                    sleep(5)
                    TT_PROC = start_tt()
...
def cleanup():
    if TT_PROC is not None:
        log_info("TrafficToll", "Killing TrafficToll...")
        TT_PROC.kill()
