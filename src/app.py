...
driver_error = lambda _driver, msg: log_error(
    f"Chrome - '{get_driver_profile(_driver)}'", msg
)
driver_info = lambda _driver, msg: log_info(
    f"Chrome - '{get_driver_profile(_driver)}'", msg
)
    ...
    try:
        ...
        if check_version or check_different_browser:
            if check_version:
                log_info(
                    src,
                    f"ChromeDriver version differs from installed Chrome version. Trying to fix that!",
                )

                try:
                    CURRENT_VERSION_MAIN = int(
                        message.split("Current browser version is ")[1]
                        .rstrip()
                        .split(".")[0]
                    )
                    ...
                    log_error(
                        src,
                        f"Could not fix that, can't get correct Chrome version: {str(e)}",
                    )
                ...
                log_info(
                    src,
                    "Unrecognized Chrome version. Are you using different browser? Trying to fix that!",
                )
                ...
                try:
                    if "unrecognized Chrome version: 1.0.0" in message:
                        ...
                        CURRENT_VERSION_MAIN = None
                        log_error(
                            "Opera is not supported, install either Google Chrome or Brave."
                        )
                        wait_before_finish()
                        exit(1)
                    else:
                    ...
                    if browser == "Edg":
                        log_error(
                            "MS Edge is not supported, install either Google Chrome or Brave."
                        )
                        wait_before_finish()
                        exit(1)
            ...
            log_info(
                src,
                f"&gSuccessfully got correct Chromium version ({CURRENT_VERSION_MAIN}) "
        ...
        if "expected str, bytes or os.PathLike object, not NoneType" in str(e):
            error(
                'Can\'t find browser executable location. &ySpecify it in config.json (field "chromium_binary"). '
...
def start_chrome(config: dict):
    ...
    info(
        f'&yBooting {len(config["profiles"])} {"headless " if config["headless"] else ""}Chrome driver(s)...'
    )
    if not config["headless"]:
        info(
            "&yFollow all instructions you see here and &rDO NOT &ypress or "
    ...
    for index, driver in enumerate(drivers):
        ...
        try:
            ...
            error(
                f'&rAuthentication check failed. You were not meant to be on URL "{driver.current_url}".'
            )
            driver.quit()
        ...
        if driver.current_url.startswith(YOUTUBE_AUTH_FAIL):
            if config["headless"]:
                ...
                driver_info(
                    driver,
                    "&rAuthentication check failed. "
                    "&mAttempting to log in using existing login information...",
                )

                SIGN_IN_XPATH = '//*[@id="view_container"]/div/div/div[2]/div/div[1]/div/form/span/section/div/div/div/div/ul/li[1]'

                # Press "Sign in" button
                driver.implicitly_wait(3)
                sign_in = driver.find_element(
                    By.XPATH, SIGN_IN_XPATH  # '//paper-button[text()="Sign in"]'
                )
                sign_in.click()

                # Press enter in the password box to log in
                driver.implicitly_wait(3)
                sign_in = driver.find_element(By.NAME, "password")
                sign_in.click()
                ...
                try:
                    WebDriverWait(driver, timeout=10).until(
                        EC.url_matches(YOUTUBE_AUTH_ANY_RE)
                    )
                except WebDriverException:
                    error(
                        f'&rAuthentication check failed. You were not meant to be on URL "{driver.current_url}".'
                ...
                if driver.current_url.startswith(YOUTUBE_AUTH_PASS):
                    driver_info(driver, "&gSuccess!\n")
                else:
                    driver_error(
                        driver,
                        "&rAuthentication check failed. "
                    ...
                    WebDriverWait(driver, 5000).until(
                        EC.url_matches(YOUTUBE_AUTH_PASS_RE)
                    )
            ...
            if index != (len(drivers) - 1) and (time() - auth_check_time) < 10:
                ...
                info(
                    f"Looks like there are more drivers. Adding {delay} seconds delay before checking for "
                    "authentication so Google doesn't complain about suspicious activity."
                )
                sleep(delay)

    info(
        "Setting stream status as &rOffline&y by default. Started looking for live stream..."
    )
    ...
    while True:
        ...
        if config["enable_owl"]:
            ...
            if url:
                ...
                if seconds and seconds > 30 * 60:
                    info(
                        "Schedule approved! Bot will start checking stream manually 30 min before the scheduled match."
                    )
            ...
            if current_url:
                ...
                for index, driver in enumerate(drivers):
                    ...
                    if index != (len(drivers) - 1):
                        delay = randint(5, 15)
                        info(
                            f"Looks like there are more drivers. Adding random {delay} seconds delay before going to "
                            f"live stream."
                        )
                        sleep(delay)
            else:
                info(
                    "&rStream has just ended :( &cTrack your token earning progress here: "
                ...
                if checks_until_reload == 0:
                    ...
                    for index, driver in enumerate(drivers):
                        ...
                        if index != (len(drivers) - 1):
                            delay = randint(5, 15)
                            info(
                                f"Looks like there are more drivers. Adding random {delay} seconds "
        ...
        if sleep_schedule:
            info(
                f"Bot is going to sleep for {round(sleep_schedule)} seconds due to enabled schedule option."
            )
    ...
    while True:
        try:
            ...
            if "no such window: target window already closed" in content:
                log_error(
                    "Bot",
                    "You closed Chrome window, app can't work without it. "
            ...
            if seconds_since_last_crash < 180:
                log_info(
                    src,
                    f"Won't try to resurrect app since "
